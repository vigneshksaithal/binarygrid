...
const canPlaceValue = (
  ...
  // Check column constraints with direct iteration
  // Optimized: Avoids allocating a temporary column array
  let zeros = 0
  let ones = 0
  for (let i = 0; i < SIZE; i++) {
    // Direct access is safe here as grid shape is validated
    const v = grid[i]![c]
    if (v === 0) {
      zeros++
    } else if (v === 1) {
  ...
  if (val === 0 && zeros >= MAX_COUNT_PER_LINE) {
    return false
  }
  if (val === 1 && ones >= MAX_COUNT_PER_LINE) {
    return false
  }
  ...
  // Check triple run in column with direct iteration
