 ...
 */
const countColumn = (grid: Grid, c: number): { zeros: number; ones: number } => {
  let zeros = 0
  let ones = 0
 ...
 * Optimized: only checks around the affected position, reading directly from grid.
 */
const wouldCreateTripleRunInColumn = (
  grid: Grid,
  r: number,
  c: number,
  val: 0 | 1
...
const canPlaceValue = (
  ...
  if (wouldCreateTripleRunInColumn(grid, r, c, val)) {
    return false
  }
