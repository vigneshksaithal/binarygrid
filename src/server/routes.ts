...
app.post('/api/submit', async (c) => {
  ...
  try {
    ...
    if (username) {
      ...
      if (!existingScore || body.score < existingScore) {
...
app.get('/api/leaderboard', async (c) => {
  ...
  const leaderboardKey = `leaderboard:${postId}`
  const scores = await redis.zRange(leaderboardKey, start, end, { by: 'rank' })

  return c.json({
    scores,
    nextCursor: scores.length === LEADERBOARD_PAGE_SIZE ? start + LEADERBOARD_PAGE_SIZE : null
  })
})
