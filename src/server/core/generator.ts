...
/**
 * Checks if the row already has the maximum allowable count of the given value.
 * Iterates through the row and counts occurrences of `val`.
 * Returns true if the count reaches MAX_COUNT_PER_LINE.
 */
const hasMaxCountRow = (row: (0 | 1 | null)[], val: 0 | 1): boolean => {
	let count = 0;
...
/**
 * Checks if the column `c` already has the maximum allowable count of the given value.
 * Accesses grid directly to avoid allocating a column array.
 */
const hasMaxCountCol = (grid: Grid, c: number, val: 0 | 1): boolean => {
	let count = 0;
	for (let r = 0; r < SIZE; r++) {
		const cell = (grid[r] as Cell[])[c];
		if (cell === val) {
			count++;
			if (count >= MAX_COUNT_PER_LINE) {
				return true;
...
/**
 * Checks if placing `val` at (r, c) would create a triple run in the column.
 * Accesses grid directly.
 */
const wouldCreateTripleRunAtCol = (
	grid: Grid,
	r: number,
	c: number,
	val: 0 | 1,
...
const canPlace = (grid: Grid, r: number, c: number, val: 0 | 1): boolean => {
	...
	if (wouldCreateTripleRunAtCol(grid, r, c, val)) {
		return false;
	}
	return true;
};
